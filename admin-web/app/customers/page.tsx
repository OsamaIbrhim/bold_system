...
export default function Customers(){
  const [phone,setPhone]=useState(''); const [list,setList]=useState<any[]>([])
  const [name,setName]=useState(''); const [whatsapp,setWhatsapp]=useState(''); const [email,setEmail]=useState('')
  const search = async()=>{ const r = await apiGet(`/customers?q=${encodeURIComponent(phone)}`); setList(Array.isArray(r)?r:r?[r]:[]) }
  ...
  <tbody>{list.map((c:any)=><tr key={c.id}><td>{c.name||'-'}</td><td>{c.phone}</td><td>{c.total_invoices||0}</td><td>{Number(c.total_spent||0)} ج</td><td>{c.is_vip?'⭐':'-'}</td><td><button className="btn" onClick={()=>toggleVip(c.id,!c.is_vip)}>{c.is_vip?'إلغاء VIP':'ترقية VIP'}</button></td></tr>)}</tbody>
  </table></div></div>)
}
