...
export default function InvoiceDetails(){
  ...
  return <div className="space-y-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><Link href="/sales" className="text-blue-700">← العودة للفواتير</Link><h1 className="text-2xl font-bold mt-2">فاتورة {invoice.invoice_number}</h1></div><div className="flex gap-2"><button className="btn" onClick={()=>pdf('ar')}>PDF عربي</button><button className="btn-secondary" onClick={()=>pdf('en')}>PDF English</button></div></div>
    <div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-7 gap-3"><div className="card"><div className="text-gray-500 text-sm">الفرع</div><b>{invoice.branch?.name_ar}</b></div><div className="card"><div className="text-gray-500 text-sm">نقطة البيع</div><b>{invoice.terminal?.name||invoice.terminal?.terminal_code||'—'}</b></div><div className="card"><div className="text-gray-500 text-sm">البائع</div><b>{invoice.seller?.name||'—'}</b></div><div className="card"><div className="text-gray-500 text-sm">الكاشير الأصلي</div><b>{invoice.cashier?.name||'—'}</b></div><div className="card"><div className="text-gray-500 text-sm">العميل</div><b>{invoice.customer?.name||invoice.customer?.phone||'نقدي'}</b></div><div className="card"><div className="text-gray-500 text-sm">الدفع</div><b>{invoice.payment_method}</b></div><div className="card"><div className="text-gray-500 text-sm">وقت البيع</div><b>{new Date(invoice.occurred_at||invoice.created_at).toLocaleString('ar-EG')}</b></div></div>
