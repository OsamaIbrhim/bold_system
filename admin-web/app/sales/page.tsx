...
export default function Sales(){
  ...
  return <div className="space-y-4">
    <div className="flex flex-wrap items-center justify-between gap-2"><div><h1 className="text-2xl font-bold">فواتير المبيعات والمرتجعات</h1><p className="text-sm text-gray-500">التاريخ المعروض هو وقت تحصيل الدفع على نقطة البيع، وليس وقت وصول المزامنة للخادم.</p></div><button className="btn-secondary" onClick={load} disabled={loading}>تحديث الآن</button></div>
    ...
    <div className="card overflow-auto">
      ...
      <table><thead><tr><th>رقم الفاتورة</th><th>الفرع</th><th>نقطة البيع</th><th>العميل</th><th>الدفع</th><th>الإجمالي</th><th>الأصناف</th><th>المرتجعات</th><th>وقت البيع</th></tr></thead><tbody>
        {data.items.map(i=><tr key={i.id}><td><Link className="text-blue-700 underline" href={`/sales/${i.id}`}>{i.invoice_number}</Link></td><td>{i.branch?.name_ar||i.branch?.code}</td><td>{i.terminal?.name||i.terminal?.terminal_code||'—'}</td><td>{i.customer?.name||i.customer?.phone||'نقدي'}</td><td>{i.payment_method}</td><td className="font-bold">{Number(i.total).toFixed(2)} ج</td><td>{i._count?.items||0}</td><td>{i._count?.original_returns||0}</td><td>{new Date(i.occurred_at||i.created_at).toLocaleString('ar-EG')}</td></tr>)}
