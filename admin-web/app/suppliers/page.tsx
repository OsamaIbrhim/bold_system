...
export default function SuppliersPage(){
  ...
  <table><thead><tr><th>الاسم</th><th>الشركة</th><th>الهاتف</th><th>Aliases</th></tr></thead>
  <tbody>{rows.map((s:any)=><tr key={s.id}><td>{s.name}</td><td>{s.company_name||'-'}</td><td>{s.phone||'-'}</td><td className="text-xs">{(s.alias_names||[]).join(', ')}</td></tr>)}
  {!rows.length && <tr><td colSpan={4} className="text-center text-gray-500 py-6">لا يوجد موردين</td></tr>}
  </tbody></table></div></div>)
}
