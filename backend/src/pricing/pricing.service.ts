...
export class PricingService {
  ...
  async calculate(variantId: string) {
    ...
    const rules = await this.prisma.pricingRule.findMany({ where: { is_active: true }, orderBy: { priority: 'asc' }});
    ...
    const selling = Math.round(price3);
    const min_allowed = Math.round(cost * (1 + oh) * (1 + tx)); // cost + protected overhead + tax
    return {
      cost,
