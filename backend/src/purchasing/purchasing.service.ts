...
export class PurchasingService {
  ...
  async receive(dto: ReceivePurchaseDto, actor: AuthenticatedUser) {
    ...
    const aggregated = new Map<
      string,
      { qty: number; totalCost: Prisma.Decimal }
    >();
    for (const item of dto.items) {
      ...
      aggregated.set(item.variant_id, {
        ...
        totalCost: current.totalCost.plus(
          new Prisma.Decimal(item.unit_cost).mul(item.qty),
        ),
      });
    }
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      for (const item of items) {
        ...
        const currentCost = new Prisma.Decimal(
          variantById.get(item.variant_id)!.cost_price,
        );
        const receivedNetUnitCost = item.unit_cost.mul(netCostMultiplier);
        const weightedCost = currentCost
          .mul(currentQty)
        ...
        if (!invoiceItem) {
          throw new NotFoundException(
            `Created purchase line is missing for variant ${item.variant_id}`,
          );
        }
        await tx.$queryRaw`
          SELECT "record_inventory_movement"(
            ...
            ${JSON.stringify({
              ...
              net_unit_cost: receivedNetUnitCost.toFixed(2),
            })}::jsonb
          )
        `;
