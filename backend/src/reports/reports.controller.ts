...
export class ReportsController {
  ...
  @Get('profit-by-item') profitByItem(@Query('from') from: string, @Query('to') to: string, @Query('branch_id') branch_id?: string) {
    return this.svc.profitByItem(from, to, branch_id);
  }
  @Get('inventory-valuation') inventoryValuation(@Query('branch_id') branch_id?: string) {
    return this.svc.inventoryValuation(branch_id);
  }
