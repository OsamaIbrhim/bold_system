      ...
      try {
        ...
        if (
          ...
          try {
            ...
            notify('تم فتح الوردية المحفوظة في وضع عدم الاتصال', 'info')
          } catch {
            setShift(null)
          }
        } else {
    ...
    api
      ...
      .then((result) => {
        ...
        if (result.session?.user?.branch_id) {
          resolveShift(result.session.user.branch_id)
        }
      })
      .catch((error) =>
        notify(
          `تعذر بدء التطبيق: ${(error as Error).message}`,
          'error',
        ),
      )
