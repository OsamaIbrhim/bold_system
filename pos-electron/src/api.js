...
export const api = {
    base: API,
    search: (q, branch_id) => fetch(`${API}/products/search?q=${encodeURIComponent(q)}${branch_id ? `&branch_id=${branch_id}` : ''}`, { headers: { ...authHeaders() } })
        .then(r => r.ok ? r.json() : []).catch(() => []),
    sale: (payload) => postJson('/pos/sale', payload),
    pricing: async (variant_id) => {
        try {
            return await postJson('/pricing/calculate', { variant_id });
        }
        catch {
            return null;
        }
    ...
    customerLookup: async (phone) => {
        try {
            return await getJson(`/customers/lookup?phone=${encodeURIComponent(phone)}`);
        }
        catch {
            return null;
        }
    },
    customerLoyalty: async (phone) => {
        try {
            return await getJson(`/customers/loyalty?phone=${encodeURIComponent(phone)}`);
        }
        catch {
...
export function calcPriceLocal(cost, overhead = 20, profit = 35, tax = 14) {
