...
function displayName(product: Product) {
  return product.name_ar || product.name_en || product.sku
}
...
>>>>>>> 27adfdb (ci: add migration-gate job and concurrency group)
    ...
    try {
      ...
      if (
        local.length === 1 &&
        [local[0].barcode_ean13, local[0].barcode_internal, local[0].sku].includes(term)
      ) {
        await addProduct(local[0])
        setResults([])
      } else {
        setResults(local)
      }
      ...
      notify('تعذر البحث في كتالوج الجهاز', 'error')
    } finally {
      setSearching(false)
      setQuery('')
  ...
  const addProduct = async (product: Product) => {
    const available = Number(await bold.stock(product.id))
    const existing = cart.find((item) => item.variant_id === product.id)

    if (existing && existing.qty >= available) {
      notify('لا توجد كمية إضافية متاحة من هذا الصنف', 'error')
      return
    }

    if (!existing && available <= 0) {
      notify('هذا المقاس غير متوفر في مخزون الفرع', 'error')
      return
    }

    let price = Number(product.selling_price || 0)
    let tax = Number(product.unit_tax || 0)

    try {
      const quote = await api.pricing(product.id)
      price = Number(quote.net_price || price)
      tax = Number(quote.tax_amount || tax)
    } catch {}

    if (price <= 0) {
      notify('لا يوجد سعر بيع معتمد لهذا الصنف', 'error')
      return
    }
    ...
    setCart((current) =>
      current.flatMap((item) => {
        ...
        if (next > item.available_qty) {
          notify(`المتاح من ${item.name}: ${item.available_qty}`, 'error')
          return [item]
        }
...
>>>>>>> 27adfdb (ci: add migration-gate job and concurrency group)
        ...
        <nav className="main-nav">
          ...
          <button onClick={onSales}>الفواتير والمرتجعات</button>
        </nav>
...
<<<<<<< HEAD
      <main className="register-layout">
        <section className="catalog-panel">
          ...
          <div className="quick-actions">
            <button onClick={() => setCustomerOpen(true)}>
              F3 · العميل{' '}
              <b>{customer?.name || customer?.phone || 'بدون عميل'}</b>
            </button>
            <button onClick={holdSale}>F4 · تعليق الفاتورة</button>
            <button onClick={() => setHeldOpen(true)}>
              الفواتير المعلقة <b>{readHeldSales().length}</b>
            </button>
          </div>
          <div className="product-results">
            {results.map((product) => (
              ...
              >
                ...
                <strong>{money(product.selling_price)} ج</strong>
              </button>
            ))}
            {!results.length && (
              <div className="catalog-empty">
                <div>⌁</div>
                <h2>جاهز للمسح</h2>
                <p>امسح باركود الصنف أو اكتب SKU ثم اضغط Enter.</p>
                <span>F2 للعودة السريعة إلى البحث</span>
              </div>
            )}
          </div>
        </section>
        ...
        <aside className="cart-panel">
          <div className="cart-heading">
            <div>
              <span className="eyebrow">الفاتورة الحالية</span>
              <h2>{totals.quantity} قطعة</h2>
            </div>
            {cart.length > 0 && (
              <button
          ...
          <div className="cart-items">
            {cart.map((item) => (
              <article className="cart-item" key={item.variant_id}>
                <div className="cart-item-main">
                  <b>{item.name}</b>
                  <span>
                    {item.sku} · {item.color || 'بدون لون'} ·{' '}
                    {item.size || 'بدون مقاس'}
                  </span>
                  <small>متاح {item.available_qty}</small>
                </div>
                <div className="qty-control">
                  <button onClick={() => changeQty(item.variant_id, item.qty - 1)}>
                    −
                  </button>
                  <input
                    value={item.qty}
                    inputMode="numeric"
                    onChange={(event) =>
                      changeQty(item.variant_id, Number(event.target.value || 0))
                    }
                  />
                  <button onClick={() => changeQty(item.variant_id, item.qty + 1)}>
                    +
                ...
                <div className="line-price">
                  <b>{money(item.unit_price * item.qty)} ج</b>
                  <span>
            ...
            {!cart.length && (
              <div className="cart-empty">
                <div>🛍</div>
                <b>السلة فارغة</b>
                <span>أضف أول صنف لبدء الفاتورة.</span>
              </div>
            )}
          </div>
          <div className="cart-summary">
            <div>
              ...
              <b>{money(totals.subtotal)} ج</b>
            </div>
            <div>
              <span>الضريبة</span>
              <b>{money(totals.tax)} ج</b>
            </div>
      ...
      <ConfirmDialog
        open={confirmClear}
        title="تفريغ السلة؟"
        message="سيتم حذف جميع الأصناف من الفاتورة الحالية."
...
function CustomerModal({
  ...
  open: boolean,
  value: Customer | null,
  onSelect: (value: Customer | null) => void,
  onClose: () => void,
  notify: (message: string, tone?: 'success' | 'error' | 'info') => void,
}) {
...
<<<<<<< HEAD
  ...
  const lookup = async () => {
    ...
    if (!isValidEgyptianPhone(normalized)) {
      setError('أدخل رقمًا مصريًا صحيحًا مثل 01012345678.')
      return
    }
    setLoading(true)
    setError('')
    try {
      const result = await api.customerLookup(normalized)
      const customer = Array.isArray(result) ? result[0] : result
      setFound(customer || null)
...
>>>>>>> 27adfdb (ci: add migration-gate job and concurrency group)
  ...
  const create = async () => {
    ...
    try {
      ...
      notify('تم إنشاء العميل', 'success')
      onSelect(customer)
    } catch (err) {
  ...
  return (
    <Modal open={open} title="العميل" onClose={onClose} width="560px">
      <div className="customer-form">
        ...
        <div className="inline-field">
          <input
            ...
            autoFocus
          />
          <button className="button secondary" onClick={lookup} disabled={loading}>
            بحث
          </button>
        </div>
        ...
        {found ? (
          <div className="customer-card">
            <div>
              <b>{found.name || 'عميل بدون اسم'}</b>
              <span dir="ltr">{found.phone}</span>
            </div>
            <div>
              <span>{found.total_invoices || 0} فاتورة</span>
              <span>{money(found.total_spent)} ج مشتريات</span>
              {found.is_vip && <strong>VIP</strong>}
            </div>
            <button className="button primary" onClick={() => onSelect(found)}>
              اختيار العميل
            </button>
          </div>
        ) : (
          <div className="new-customer">
            <label>اسم العميل الجديد (اختياري)</label>
            <input
              value={name}
            ...
            <button className="button primary" onClick={create} disabled={loading}>
              إنشاء واختيار العميل
            </button>
          </div>
        )}
        <button className="button ghost full" onClick={() => onSelect(null)}>
          إكمال البيع بدون عميل
        </button>
      </div>
    </Modal>
  )
...
function CheckoutModal({
  ...
  const change = Math.max(0, receivedValue - totals.total)

  const confirm = async () => {
    if (busy) return

    if (method === 'cash' && receivedValue < totals.total) {
    ...
    try {
      const saved = await bold.sale(payload)
      const receipt = {
        invoice_number: `POS-${saved.sync_id.slice(0, 8).toUpperCase()}`,
        total: totals.total,
        subtotal: totals.subtotal,
        tax: totals.tax,
        payment_method: method,
        received: method === 'cash' ? receivedValue : undefined,
        change: method === 'cash' ? change : undefined,
        items,
      }
      const printResult = await bold
        .print(receipt, 'ar')
        .catch((printError) => ({
          ok: false,
          reason: (printError as Error).message,
        }))
      onCompleted({
        ...receipt,
        sync_id: saved.sync_id,
        printed: !!printResult?.ok,
        print_error: printResult?.reason,
      })
      notify('تم حفظ البيع محليًا بأمان', 'success')
    } catch (err) {
      const value = err as Error
      setError(value.message || 'تعذر حفظ البيع محليًا')
      setBusy(false)
    }
  }
  ...
  return (
    ...
    >
      <div className="checkout-layout">
        <section>
          <div className="checkout-total">
            ...
            <b>{money(totals.total)} ج</b>
            <small>
              {totals.quantity} قطعة · ضريبة {money(totals.tax)} ج
            </small>
          </div>
          ...
          {method === 'cash' && (
            <>
              ...
              <div className="cash-presets">
                <button onClick={() => setReceived(String(totals.total))}>
                  المبلغ بالضبط
                </button>
                {[50, 100, 200, 500, 1000]
                  .filter((value) => value >= totals.total)
                  .slice(0, 4)
                  .map((value) => (
                    <button key={value} onClick={() => setReceived(String(value))}>
                      {value}
                    </button>
                  ))}
              </div>
      ...
      <div className="dialog-actions">
        ...
        <button className="button primary xl" disabled={busy} onClick={confirm}>
          {busy
            ? 'جارٍ حفظ البيع…'
            : `تأكيد ${paymentLabel(method)}`}
        </button>
      </div>
    </Modal>
  )
...
function HeldSalesModal({
  ...
  return (
    <Modal open={open} title="الفواتير المعلقة" onClose={onClose} width="720px">
      <div className="held-list">
        {sales.map((sale) => (
          <article key={sale.id}>
            <div>
              <b>{sale.customer?.name || sale.customer?.phone || 'بدون عميل'}</b>
              <span>{new Date(sale.created_at).toLocaleString('ar-EG')}</span>
            </div>
            <div>
              <b>
                {sale.items.reduce(
              ...
              <span>{money(cartTotals(sale.items).total)} ج</span>
            </div>
            <button className="button primary" onClick={() => onResume(sale)}>
              استكمال
            </button>
            <button
        ...
        {!sales.length && (
          <div className="empty-state">
            <b>لا توجد فواتير معلقة</b>
            <span>استخدم F4 لتعليق الفاتورة الحالية.</span>
          </div>
        )}
      </div>
    </Modal>
...
function SaleSuccessModal({
  ...
  return (
    <Modal open={!!value} title="تمت العملية" onClose={onClose} width="540px">
      {value && (
        <div className="success-state">
          <div className="success-icon">✓</div>
          <h2>تم حفظ البيع</h2>
          <b className="success-total">{money(value.total)} ج</b>
          <div className="receipt-meta">
            <span>رقم العملية</span>
            <code>{value.sync_id}</code>
            <span>الطباعة</span>
            <b className={value.printed ? 'ok' : 'warn'}>
              {value.printed ? 'تمت الطباعة' : 'لم تتم الطباعة'}
            </b>
            {value.change !== undefined && (
              <>
                <span>الباقي</span>
                <b>{money(value.change)} ج</b>
              </>
            )}
          </div>
          {!value.printed && (
            <FieldError>
              {value.print_error ||
                'يمكن إعادة الطباعة من سجل الفواتير بعد المزامنة.'}
            </FieldError>
          )}
          <button className="button primary xl full" onClick={onClose}>
            بيع جديد
          </button>
        </div>
      )}
