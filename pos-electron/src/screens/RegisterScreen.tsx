...
export function RegisterScreen({
  ...
  const addProduct=async(product:Product)=>{
    ...
    let price=Number(product.selling_price||0),tax=Number(product.unit_tax||0)
    try{const quote=await api.pricing(product.id);price=Number(quote.net_price||price);tax=Number(quote.tax_amount||tax)}catch{}
    if(price<=0){notify('لا يوجد سعر بيع معتمد لهذا الصنف','error');return}
